...
int main() {
	...
	a = s + baseChar + t;
	getLCP(suffixArray, LCPArray, a);

	/*cout << "SA: "; for (int i = 0; i < a.length(); ++i) cout << setw(2) << suffixArray[i] << " "; cout << "\n";
	cout << "LA: "; for (int i = 0; i < a.length(); ++i) cout << setw(2) << LCPArray[i] << " "; cout << "\n";*/

	int res = 0, resp = 0;
	for (int i = 0; i < LCPArray.size() - 1; ++i) {
		//if (suffixArray[i] == 0) continue;
		if ((suffixArray[i] > s.length()) != (suffixArray[i + 1] > s.length())) {
			int tmp = res;
			res = max(res, LCPArray[i + 1]);
			if (res > tmp) resp = suffixArray[i];
		}
	}
	cout << res << "\n";
	for (int i = 0; i < res; ++i) cout << a[resp + i];
